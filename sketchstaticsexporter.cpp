...
QVariant SketchStaticsExporter::saveFile(QString basename, QString backgroundImagePath, QSize appSize , QString path) {
    ...
    QString filename = path.remove(0, 7) + '/' + basename;
    QFile file(filename + ".carp");
    ...
    foreach(QVariant rawPoint, points) {
        ...
        if(cx || cy || cz || mx || my || mz) {
            numberOfReaction++;
            if(i==0 && !atLeastOneSupport){ // kind of test
    ...
    const QImage backgroundImage(backgroundImagePath);
    QImage outputImage;
    QPainter painter;
    int xOffset=0;
    int width=640;
...
QVariant SketchStaticsExporter::exportToFile(QString basename, QString backgroundImagePath, QSize appSize , QString path) {
    // Check if the path/basename.carp is already used, if not saveFile, else choose another basename
}
